...
Begin
  ...
  Try
   ...
   EdUserNameBD.Text       := Ini.ReadString('BancoDados', 'UsuarioBD',  '');
   EdPasswordBD.Text       := Ini.ReadString('BancoDados', 'SenhaBD',    '');
   EdPortaDW.Text          := Ini.ReadString('BancoDados', 'PortaDW',    '8082');
   EdUserNameDW.Text       := Ini.ReadString('BancoDados', 'UsuarioDW',  '');
   EdPasswordDW.Text       := Ini.ReadString('BancoDados', 'SenhaDW',    '');
...
//   End;
   ...
   { A janela sumia para a bandeja aqui, assim que o servidor subia. No
     Windows 11 ela nao volta, e o programa fica sem como ser parado. O
     metodo continua existindo - o duplo clique no icone da bandeja ainda
     chama ShowApplication. }
//   HideApplication;
