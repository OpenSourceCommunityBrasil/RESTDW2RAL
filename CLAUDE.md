...
There is also `ferramentas/conversor`: `uConversor.pas` holds every rule and **three shells** stand over it - `rdw2ral.dpr` (console), `gui/rdw2ralgui.dpr` (Delphi window) and `gui-lazarus/rdw2ralgui.lpi` (Lazarus window). **Change the rule in the unit, never in a shell**, or they drift apart. A window is the path a migrating user is meant to take; the console is for scripting. The engine compiles on both compilers, which is why it carries no `System.IOUtils` and no generics: the file helpers at the top of the implementation are the replacement, and `PacotesDoIDE` is the one place that differs - the Delphi registry on one side, Lazarus's `staticpackages.inc` on the other, answering the same question about which engine packages the user has installed.
...
- **A failure has two ways to arrive, and both count.** It is an exception when `RaiseErrors` is on and a message stored by `InternalError` when it is off, so any place that reports a failure has to look at both - `ComErro` is the one that does, for `ExecSQL(var AError)`, `ApplyUpdates(var AError)` and `AplicarPendentes`. Reading only the exception meant that **turning `RaiseErrors` off made every database refusal disappear**: the function answered True with an empty error and the caller went on believing it had written. In RDW `RaiseErrors` decides whether there is an exception, not whether the error is reported. `Open`, `Post` and `Delete` have nowhere to return one, so they leave it in the public `LastError`, which every operation clears as it starts. Measured with a harness that provokes a failure in each operation, in both modes: nothing is silent now.
- **Clear the stored error where the request starts, never where the wait starts.** `WaitResponse` used to begin with `FLastError := ''`, which is right only while everything is asynchronous. `ExecSQL` and `ApplyUpdates` go with `ebSingleThread`: the callback - and with it `InternalError` - has already run inside `inherited`, so the wait wiped the message before raising it and **every failed statement came back as success**. A migrated project reading `ApplyUpdates(var AError)` then saw True with an empty error and nothing written. `IniciarRequisicao` resets both flags at the three dispatch points instead. Surfacing these errors is RDW parity, and it is loud: `ExecSQL` of a SELECT now answers `-310 Cannot execute command returning result sets`, because that is what FireDAC on the server says.
